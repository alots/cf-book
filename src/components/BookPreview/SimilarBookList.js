...
const SimilarBookList = ({ similarBooks }) => {
  const visibleBooksInit = similarBooks.slice(
    0,
    settings.visibleSimilarBooksCount
  );
  const hiddenBooksInit = similarBooks.slice(
    settings.visibleSimilarBooksCount,
    similarBooks.length
  );
  ...
  const changeSimilarBooks = useCallback((currentId) => {
    setState((prevState) => {
      let newState = {};
      newState.visibleBooks = prevState.visibleBooks.filter(
  ...
  return state.visibleBooks.map((book) => (
    <SimilarBookCard
      key={book.id}
      book={book}
      changeSimilarBooks={changeSimilarBooks}
    />
  ));
};
